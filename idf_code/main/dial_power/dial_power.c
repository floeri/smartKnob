...
void adc_read_task() {
  ...
  adc_continuous_handle_t handle = NULL;
  continuous_adc_init(adc_channel, sizeof(adc_channel) / sizeof(adc_channel_t),
                      &handle);
  ...
  while (1) {
    ...
    while (1) {
      ...
      if (ret == ESP_OK) {
        ...
        // ret_num); for (int i = 0; i < ret_num; i +=
