...
static const foc_knob_param_t press_foc_knob_param = {
    0, 0, 1 * PI / 180, 2, 1, 1.1, ""}; //按下旋转的力度
static const foc_knob_param_t press_foc_knob_param2 = {
    0, 0, 2 * PI / 180, 2, 1, 1.1, ""}; //按下旋转的力度
static const foc_knob_param_t press_foc_knob_param3 = {
    0, 0, 4 * PI / 180, 2, 1, 1.1, ""}; //按下旋转的力度
static const foc_knob_param_t press_foc_knob_param4 = {
    0, 0, 8 * PI / 180, 4, 1, 1.1, ""}; //按下旋转的力度
static const foc_knob_param_t press_foc_knob_param5 = {
    0, 0, 16 * PI / 180, 2, 1, 1.1, ""}; //按下旋转的力度
static const foc_knob_param_t press_foc_knob_param6 = {
    0, 0, 32 * PI / 180, 0.5, 1, 1.1, ""}; //按下旋转的力度
static const foc_knob_param_t press_foc_knob_param7 = {
    0, 0, 64 * PI / 180, 0.5, 1, 1.1, ""}; //按下旋转的力度

static const foc_knob_param_t press_foc_knob_param8 = {
    4, 0, 64 * PI / 180, 0, 10, 1.1, ""}; //按下旋转的力度

static const foc_knob_param_t press_foc_knob_param9 = {
    0, 0, 64 * PI / 180, 0, 10, 1.1, ""}; //按下旋转的力度

int mode = 0;

void set_screen3_index() { nvs_set_u8_data(NVS_CSM_HID_INDEX, icon_index); }
static void ui_icon_info_init() {
  ...
  ui_icon_hid[id].icon.name = "Demo";
  
  ui_icon_hid[id].icon.left_info = "";
  ui_icon_hid[id].icon.mid_info = "~ one Knob to rule them all ~";
...
void ui_Screen3_Custom_hid_event(uint8_t state) {
  ...
  case DIAL_STA_RELEASE:
    ...
    case 1:
      foc_knob_set_param(press_foc_knob_param2);
      break;
    case 2:
      foc_knob_set_param(press_foc_knob_param3);
      break;
    case 3:
      foc_knob_set_param(press_foc_knob_param4);
      break;
    case 4:
      foc_knob_set_param(press_foc_knob_param5);
      break;
    case 5:
      foc_knob_set_param(press_foc_knob_param6);
      break;
    case 6:
      foc_knob_set_param(press_foc_knob_param7);
      break;
    case 7:
      foc_knob_set_param(press_foc_knob_param8);
      break;
    }
    break;
  case DIAL_STA_PRESS:
    // hid_send(state);
      foc_knob_set_param(press_foc_knob_param9);
...
static void Style_Init() {
  ...
  lv_style_transition_dsc_init(&trans, style_prop, lv_anim_path_bounce, 200, 0,
                               NULL);
...
static void Create(lv_obj_t *root) {
  ...
  lv_obj_set_style_img_recolor(ble_img, lv_color_hex(0x000000),
                               LV_PART_MAIN | LV_STATE_DEFAULT);
  lv_obj_set_style_img_recolor_opa(ble_img, 255,
                                   LV_PART_MAIN | LV_STATE_DEFAULT);
...
void ui_Screen3_screen_init(void) {
  ...
  ui_icon_info_init();
  lv_obj_add_event_cb(ui_Screen3, scr_Screen3_loaded_cb, LV_EVENT_SCREEN_LOADED,
                      &ui_Screen3);
  // lv_obj_add_event_cb(ui_Screen3, scr_Screen2_unloaded_cb,
  // LV_EVENT_SCREEN_UNLOADED, &ui_Screen3);
  lv_obj_clear_flag(ui_Screen3, LV_OBJ_FLAG_SCROLLABLE); /// Flags
  lv_obj_set_style_bg_img_src(ui_Screen3, &ui_img_bg1_png,
                              LV_PART_MAIN | LV_STATE_DEFAULT);
  Style_Init();
  Create(ui_Screen3);
  setl_label_info(icon_index);
